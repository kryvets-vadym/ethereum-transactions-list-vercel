...
import { LoadBlocksParams } from '../types/types.js';
...
export const fetchBlocks = async (number: string, latestBlockNumber: string) => {
  try {
    await sleep(1000);
...
export const loadBlocks = async ({ count, number, lastBlockNumber }: LoadBlocksParams) => {
  try {
    const {
      REQUESTS_PER_SECOND,
      SLEEP_TIME,
      NUMBER_OF_BLOCKS_FOR_INIT,
    } = config;
    ...
    const numbersOfBlocks = Array.from(
